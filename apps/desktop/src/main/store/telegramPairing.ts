...
export function getOrCreateTelegramPairing(userId: string): { code: string; created: boolean } {
  pruneExpired();
  const requests = pairingStore.get('requests') ?? {};
...
export function approveTelegramPairing(userId: string, code: string): boolean {
  pruneExpired();
  const requests = pairingStore.get('requests') ?? {};
  const existing = requests[userId];
  if (!existing) {
...
export function listTelegramPairingRequests(): PairingRequest[] {
  pruneExpired();
  const requests = pairingStore.get('requests') ?? {};
